...
const DAY_MAP_NUM_TO_STR: { [k: number]: string } = {
  1: 'MON',
  2: 'TUE',
  3: 'WED',
  4: 'THU',
...
const DAY_MAP_STR_TO_NUM: { [k: string]: number } = {
  MON: 1,
  TUE: 2,
  WED: 3,
  THU: 4,
  ...
  try {
    ...
    const schedules = result.rows.map((row) => {
      const days = Array.isArray(row.daysOfWeekRaw)
        ? row.daysOfWeekRaw.map((n: number | string) =>
            typeof n === 'number' ? DAY_MAP_NUM_TO_STR[n] || String(n) : String(n)
          )
        : ['MON', 'TUE', 'WED', 'THU', 'FRI'];
...
export const createSchedule = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const numDays: number[] = Array.isArray(daysOfWeek)
      ? daysOfWeek.map((d: string | number) =>
          typeof d === 'string' ? DAY_MAP_STR_TO_NUM[d.toUpperCase()] || 1 : Number(d)
        )
      : [1, 2, 3, 4, 5];
...
export const updateSchedule = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const numDays: number[] | null = Array.isArray(daysOfWeek)
      ? daysOfWeek.map((d: string | number) =>
          typeof d === 'string' ? DAY_MAP_STR_TO_NUM[d.toUpperCase()] || 1 : Number(d)
        )
      : null;
