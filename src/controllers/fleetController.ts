  ...
  try {
    ...
    if (compliance && typeof compliance === 'object') {
      for (const [docType, expiryDate] of Object.entries(compliance)) {
        if (expiryDate) {
          await query(
            `INSERT INTO core.vehicle_documents (vehicle_id, doc_type, file_path, issued_at, expires_at, is_verified)
             VALUES ($1, $2, $3, CURRENT_DATE - INTERVAL '6 months', $4, true)
             ON CONFLICT DO NOTHING`,
            [newVehicle.id, docType, `/docs/vehicles/${newVehicle.id}/${docType}.pdf`, expiryDate]
          );
        }
      }
    }
        ...
        if (expiryDate) {
          await query(
            `INSERT INTO core.vehicle_documents (vehicle_id, doc_type, file_path, issued_at, expires_at, is_verified)
             VALUES ($1, $2, $3, CURRENT_DATE - INTERVAL '6 months', $4, true)
             ON CONFLICT DO NOTHING`,
            [id, docType, `/docs/vehicles/${id}/${docType}.pdf`, expiryDate]
          );
        }
      }
    }
...
export const getDriverAssignments = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const sql = `
      SELECT 
        da.id,
