...
export const getHalts = async (req: Request, res: Response, next: NextFunction) => {
  try {
...
export const createHalt = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (distRes.rows.length > 0) {
      ...
      const newDist = await query(
        'INSERT INTO core.districts (name, province) VALUES ($1, $2) RETURNING id',
        [distName, provName]
      );
