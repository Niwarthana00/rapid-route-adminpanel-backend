  ...
  try {
    ...
    if (pRes.rows.length > 0) {
      ...
      const newP = await query(
        `INSERT INTO core.passengers (full_name, phone, email) VALUES ($1, $2, $3) RETURNING id`,
        [pName, pPhone, passengerEmail || null]
      );
      passengerId = newP.rows[0].id;
      await query(
        `INSERT INTO core.passenger_loyalty (passenger_id, tier, total_trips, total_spent, points_balance)
         VALUES ($1, 'BRONZE', 0, 0, 0) ON CONFLICT DO NOTHING`,
        [passengerId]
      );
    }
    ...
    await query(
      ...
      VALUES ($1, 'CASH', $2, 'LKR', $3, 'SUCCESS', NOW())`,
      [created.id, fare, `TXN-${Date.now().toString().slice(-8)}`]
    );
...
export const getSeatMap = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    let targetVehicleId = vehicleId as string;
    ...
    // Get vehicle seats
    const seatMapRes = await query(
      `SELECT seat_number, seat_type FROM core.seat_map WHERE vehicle_id = $1 ORDER BY seat_number ASC`,
      [targetVehicleId]
    );

    // Get active bookings for trip
    let bookedSeats: { [k: number]: string } = {};
    if (tripId) {
      const bookRes = await query(
        `SELECT b.seat_number, p.full_name 
         FROM biz.bookings b
         JOIN core.passengers p ON p.id = b.passenger_id
         WHERE b.trip_id = $1 AND b.booking_status NOT IN ('CANCELLED')`,
        [tripId]
      );
