...
export const getDashboardSummary = async (req, res, next) => {
    try {
        ...
        const inServiceRes = await query(`SELECT COUNT(*)::INT AS count FROM core.vehicles WHERE is_active = true AND deleted_at IS NULL`);
        const inServiceVehicles = inServiceRes.rows[0]?.count || 0;
        // 2. Active buses (on-road right now)
        const activeBusesRes = await query(`SELECT COUNT(DISTINCT vehicle_id)::INT AS count FROM biz.trips 
       WHERE status IN ('BOARDING', 'DEPARTED', 'IN_PROGRESS', 'DELAYED') 
        ...
        const passengerRes = await query(`SELECT COUNT(b.id)::INT AS count FROM biz.bookings b
       ...
       WHERE (t.trip_date = CURRENT_DATE OR b.booked_at::DATE = CURRENT_DATE) 
        ...
        const revenueRes = await query(`SELECT COALESCE(SUM(p.amount), 0)::NUMERIC AS total 
       FROM fin.payments p
       ...
       WHERE (t.trip_date = CURRENT_DATE OR p.paid_at::DATE = CURRENT_DATE) 
        ...
        const outOfServiceRes = await query(`SELECT COUNT(*)::INT AS count FROM core.vehicles WHERE is_active = false AND deleted_at IS NULL`);
        const outOfServiceCount = outOfServiceRes.rows[0]?.count || 0;
        // 6. Delayed trips count
        const delayedTripsRes = await query(`SELECT COUNT(*)::INT AS count FROM biz.trips 
       WHERE (status = 'DELAYED' OR delay_reason IS NOT NULL) AND trip_date = CURRENT_DATE`);
        ...
        const driverDocsExpiring = await query(`SELECT COUNT(*)::INT AS count FROM core.drivers 
       WHERE license_expiry <= CURRENT_DATE + INTERVAL '30 days' AND deleted_at IS NULL`);
        const vehicleDocsExpiring = await query(`SELECT COUNT(*)::INT AS count FROM core.vehicle_documents 
       WHERE expires_at <= CURRENT_DATE + INTERVAL '30 days'`);
        const expiringDocsCount = (driverDocsExpiring.rows[0]?.count || 0) + (vehicleDocsExpiring.rows[0]?.count || 0);
        res.json({
...
export const dispatchReplacement = async (req, res, next) => {
    try {
