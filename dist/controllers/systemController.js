...
export const getHeaderNotifications = async (req, res, next) => {
    try {
        // Return urgent alerts such as delayed buses, vehicle compliance expiries, maintenance items
        const alerts = [];
        // Check delayed trips
        const delayed = await query(`SELECT t.id, r.route_number, t.delay_reason FROM biz.trips t
       ...
       LIMIT 2`);
        ...
        const expiringDocs = await query(`SELECT d.full_name, d.license_number, d.license_expiry::TEXT 
       FROM core.drivers d 
