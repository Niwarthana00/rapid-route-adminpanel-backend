...
export const getFareRules = async (req, res, next) => {
    try {
...
export const createFareRule = async (req, res, next) => {
    try {
        const { routeId, fromHaltId, toHaltId, baseFareLkr, perKmRateLkr, acSurcharge, acSurchargeLkr, effectiveFrom, effectiveTo, } = req.body;
    ...
    `;
        ...
        // Fetch names
        const names = await query(`SELECT r.route_number, h1.name AS from_name, h2.name AS to_name, r.total_distance_km
       FROM core.routes r, core.halts h1, core.halts h2
       WHERE r.id = $1 AND h1.id = $2 AND h2.id = $3`, [routeId, fromHaltId, toHaltId]);
...
export const updateFareRule = async (req, res, next) => {
    try {
        const { id } = req.params;
        const { baseFareLkr, perKmRateLkr, acSurcharge, acSurchargeLkr, effectiveFrom, effectiveTo, } = req.body;
        const sql = `
...
export const getRevenueFacts = async (req, res, next) => {
    try {
        const sql = `
      SELECT 
        ftr.id,
    ...
    `;
      ...
      `;
            ...
            return res.json(dynRes.rows.map((row) => ({
                ...row,
