...
export const getTripHaltLogs = async (req, res, next) => {
    try {
        const { tripRef } = req.params;
        // Find trip by ID or by ref
        let tripId = tripRef;
        if (tripRef.startsWith('TRP-')) {
            const tripMatch = await query(`SELECT id FROM biz.trips WHERE 'TRP-' || TO_CHAR(trip_date, 'YYYYMMDD') || '-' || LPAD(SUBSTRING(id::TEXT, 1, 4), 4, '0') = $1 LIMIT 1`, [tripRef]);
